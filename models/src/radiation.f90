...
CONTAINS
  ...
  SUBROUTINE READ_RAD_GRID
    ...
    INTEGER            :: file_grid   ! unit to read grid file
    ...
    REAL(KIND=dp)      :: wl          ! wavelength (in A)
  ...
  SUBROUTINE INTERP_SIGMA_G_RF
     ...
     DO k = 1, Ncross
        ...
        DO j = 1, npts-1
           ...
           IF(i1 == i2) CYCLE
           DO i = i1, i2
              sigma_G_rf(k,i) = sigma_G_rf(k,i) + 1.0_dp / ( wl2 - wl1 ) &
                              * ( sig1 * ABS(wlg(i) - wl2) &
                                + sig2 * ABS(wlg(i) - wl1) )
           ENDDO
        ENDDO
  ...
  SUBROUTINE FGKCOEF
    ...
    !     fgkgrd
    !---------------------------------------------------------------------------
    USE MODULE_CONSTANTS,        ONLY : pi, sqpi, kB, qe, me, clum, iwrtmp
    USE MODULE_TECHCONFIG
    USE MODULE_PHYS_VAR
    ...
    IF (F_COUP_RAD == 1) THEN
       ...
       DO i = 1, uvh2_bx%nuv
          ...
          fgk_h2bx_l(i) = MIN(AJDF(taud_l) + ajr_l, 1.0_dp)
          fgk_h2bx_r(i) = MIN(AJDF(taud_r) + ajr_r, 1.0_dp)
       ENDDO
       ...
       DO i = 1, uvh2_cx%nuv
          ...
          fgk_h2cx_l(i) = MIN(AJDF(taud_l) + ajr_l, 1.0_dp)
          fgk_h2cx_r(i) = MIN(AJDF(taud_r) + ajr_r, 1.0_dp)
       ENDDO
       ...
       DO i = 1, uvco%nuv
          ...
          fgk_co_l(i) = MIN(AJDF(taud_l) + ajr_l, 1.0_dp)
          fgk_co_r(i) = MIN(AJDF(taud_r) + ajr_r, 1.0_dp)
       ENDDO
