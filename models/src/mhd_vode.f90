PROGRAM MHD
  ...
  USE MODULE_GRAINS,            ONLY : compr_gr
  ...
  USE MODULE_H2,                ONLY : H2_lev, Index_VJ_H2, H2_lines, NH2_lines, op_LTE
  ...
  USE MODULE_VAR_VODE,          ONLY : Eps_V, T0_V, H0_V, &
  ...
  USE MODULE_ENERGETICS,        ONLY : ENERGETIC_FLUXES, mag_flux_corr
  ...
  ! DO i = 1, Nelements
  !    WRITE(*,*) elements(i)%name, elements(i)%ab_init, elements(i)%Dens_init
  ! ENDDO
  ! STOP
  ...
  i = 21 ; v_variab(i) = compr_gr   ; iv_compr_gr   = i ! grain compression factor
  !!!!! i = 22 ; v_variab(i) = d_ero      ; iv_d_ero      = i ! erosion size
  !!!!! i = 23 ; v_variab(i) = d_ads      ; iv_d_ads      = i ! adsorption size
  ...
  lrw_V = 22 + 9*d_v_var + 2*d_v_var**2  ! taille du tableau rwork_V
  ...
  DO slab = 1, nposgrd
     ...
     DO WHILE ( again .and. t0_v < Tout_V )
        ...
        DO WHILE (.not. its_OK)
           ...
           !-------------------------------------------------
           CALL diffun(d_v_var,Tout_V,v_lvariab,v_dvariab)
           dVn = v_dvariab(iv_Vn)
           dVi = v_dvariab(iv_Vi)

           !------------------------------------------------------------------
           ! In subroutines MAIN_CARRIER and INDEX_CONSERVATION, the main
        ...
        !=================================================================================
        ! 4b - Numerical Integration - post-treatment
        !      Insure conservations (NEED TO BE OPTIONAL)
        !      compute additional variables (distance, column densities, intensities)
        !      check criteria to stop integration
        !=================================================================================

        !-------------------------------------------------
        ! Force conservation of elements and charge
        ...
        !            + SUM(DBLE(speci(b_gra:e_gra)%density)) &
        !            + SUM(DBLE(speci(b_cor:e_cor)%density))  
        ...
        !        + DOT_PRODUCT(DBLE(speci(b_gra:e_gra)%density), DBLE(speci(b_gra:e_gra)%mass)) &
        !        + DOT_PRODUCT(DBLE(speci(b_cor:e_cor)%density), DBLE(speci(b_cor:e_cor)%mass))
        ...
        ! v_lvariab(bv_specy+1:bv_specy+Nspec) = log(speci(1:Nspec)%density)
        ! v_lvariab( iv_DensityN   ) = log( DensityN   )
        ! v_lvariab( iv_DensityI   ) = log( DensityI   )
        ! v_lvariab( iv_DensityA   ) = log( DensityA   )
        ! v_lvariab( iv_DensityNeg ) = log( DensityNeg )
        ! v_lvariab( iv_RhoN       ) = log( RhoN       )
        ! v_lvariab( iv_RhoI       ) = log( RhoI       )
        ! v_lvariab( iv_RhoA       ) = log( RhoA       )
        ! v_lvariab( iv_RhoNEG     ) = log( RhoNEG     )
        ! CALL diffun(d_v_var,Tout_V,v_lvariab,v_dvariab)
        !-------------------------------------------------
        ...
        !----------------------------------------------------
        CALL TEST_COUPLING(restart) !!!!!Tram
  ...
  !    - line emissions
  !-------------------------------------------------------
  IF (shock_type(1:1) /= 'S' .AND. shock_type(1:1) /= 'P') THEN
