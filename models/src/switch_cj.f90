...
CONTAINS
  ...
  SUBROUTINE INTERP_JUMP_CONDITIONS(t_jump,i_jump)
    ...
    REAL(KIND=DP)                                 :: compr_gr_1   , compr_gr_2
    ...
    ! compr_gr    = LOG(ABS(compr_gr_1   )) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(compr_gr_1   )) ) * dum
    ! fluph       = LOG(ABS(fluph_1      )) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(fluph_2      )) ) * dum
    ! coldens_h   = LOG(ABS(coldens_h_1  )) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(coldens_h_2  )) ) * dum
    ! coldens_h2  = LOG(ABS(coldens_h2_1 )) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(coldens_h2_2 )) ) * dum
    ! coldens_co  = LOG(ABS(coldens_co_1 )) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(coldens_co_2 )) ) * dum
    ! Col_Dens_nH = LOG(ABS(Col_Dens_nH_1)) + ( LOG(ABS(compr_gr_2  )) - LOG(ABS(Col_Dens_nH_2)) ) * dum
    ! IF (F_AV /= 0) THEN
    !    DO i = 1, nangle
    ...
    ! compr_gr    = SIGN( EXP(compr_gr   ) , compr_gr_1   )
  ...
  SUBROUTINE AVERAGE_TRAJEC(traj1, traj2, trajf, N, Ntraj2, ka, kb, adjst)
    ...
    DO k1 = ka, kb
       ...
       trajf(k1)%compr_gr     = AVER(traj1(k1)%compr_gr     ,traj2(k2)%compr_gr     ,traj2(k2+1)%compr_gr     ,dum)
       ...
       ! heating
       !---------------------------------------------------------------------------
       trajf(k1)%heat_n_tot     = AVER(traj1(k1)%heat_n_tot    ,traj2(k2)%heat_n_tot    ,traj2(k2+1)%heat_n_tot    ,dum)
       trajf(k1)%heat_n_chem    = AVER(traj1(k1)%heat_n_chem   ,traj2(k2)%heat_n_chem   ,traj2(k2+1)%heat_n_chem   ,dum)
       trajf(k1)%heat_n_diff_in = AVER(traj1(k1)%heat_n_diff_in,traj2(k2)%heat_n_diff_in,traj2(k2+1)%heat_n_diff_in,dum)
       trajf(k1)%heat_n_diff_an = AVER(traj1(k1)%heat_n_diff_an,traj2(k2)%heat_n_diff_an,traj2(k2+1)%heat_n_diff_an,dum)
       trajf(k1)%heat_n_diff_en = AVER(traj1(k1)%heat_n_diff_en,traj2(k2)%heat_n_diff_en,traj2(k2+1)%heat_n_diff_en,dum)
       trajf(k1)%heat_n_diff_gn = AVER(traj1(k1)%heat_n_diff_gn,traj2(k2)%heat_n_diff_gn,traj2(k2+1)%heat_n_diff_gn,dum)
       trajf(k1)%heat_n_inel_en = AVER(traj1(k1)%heat_n_inel_en,traj2(k2)%heat_n_inel_en,traj2(k2+1)%heat_n_inel_en,dum)
       trajf(k1)%heat_n_ther_in = AVER(traj1(k1)%heat_n_ther_in,traj2(k2)%heat_n_ther_in,traj2(k2+1)%heat_n_ther_in,dum)
       trajf(k1)%heat_n_ther_gn = AVER(traj1(k1)%heat_n_ther_gn,traj2(k2)%heat_n_ther_gn,traj2(k2+1)%heat_n_ther_gn,dum)
       trajf(k1)%heat_n_ph      = AVER(traj1(k1)%heat_n_ph     ,traj2(k2)%heat_n_ph     ,traj2(k2+1)%heat_n_ph     ,dum)
       trajf(k1)%heat_n_cr      = AVER(traj1(k1)%heat_n_cr     ,traj2(k2)%heat_n_cr     ,traj2(k2+1)%heat_n_cr     ,dum)
       trajf(k1)%heat_n_phchem  = AVER(traj1(k1)%heat_n_phchem ,traj2(k2)%heat_n_phchem ,traj2(k2+1)%heat_n_phchem ,dum)
       trajf(k1)%heat_n_h2int   = AVER(traj1(k1)%heat_n_h2int  ,traj2(k2)%heat_n_h2int  ,traj2(k2+1)%heat_n_h2int  ,dum)
       trajf(k1)%heat_n_visc    = AVER(traj1(k1)%heat_n_visc   ,traj2(k2)%heat_n_visc   ,traj2(k2+1)%heat_n_visc   ,dum)
       trajf(k1)%heat_n_compr   = AVER(traj1(k1)%heat_n_compr  ,traj2(k2)%heat_n_compr  ,traj2(k2+1)%heat_n_compr  ,dum)

       !---------------------------------------------------------------------------
       ! cooling
       !---------------------------------------------------------------------------
       trajf(k1)%cool_n_tot   = AVER(traj1(k1)%cool_n_tot   ,traj2(k2)%cool_n_tot   ,traj2(k2+1)%cool_n_tot   ,dum)
       trajf(k1)%cool_n_h2    = AVER(traj1(k1)%cool_n_h2    ,traj2(k2)%cool_n_h2    ,traj2(k2+1)%cool_n_h2    ,dum)
       trajf(k1)%cool_n_13co  = AVER(traj1(k1)%cool_n_13co  ,traj2(k2)%cool_n_13co  ,traj2(k2+1)%cool_n_13co  ,dum)
       trajf(k1)%cool_n_oh    = AVER(traj1(k1)%cool_n_oh    ,traj2(k2)%cool_n_oh    ,traj2(k2+1)%cool_n_oh    ,dum)
       trajf(k1)%cool_n_nh3   = AVER(traj1(k1)%cool_n_nh3   ,traj2(k2)%cool_n_nh3   ,traj2(k2+1)%cool_n_nh3   ,dum)
       trajf(k1)%cool_n_co    = AVER(traj1(k1)%cool_n_co    ,traj2(k2)%cool_n_co    ,traj2(k2+1)%cool_n_co    ,dum)
       trajf(k1)%cool_n_h2o   = AVER(traj1(k1)%cool_n_h2o   ,traj2(k2)%cool_n_h2o   ,traj2(k2+1)%cool_n_h2o   ,dum)
       trajf(k1)%cool_n_cp    = AVER(traj1(k1)%cool_n_cp    ,traj2(k2)%cool_n_cp    ,traj2(k2+1)%cool_n_cp    ,dum)
       trajf(k1)%cool_n_sip   = AVER(traj1(k1)%cool_n_sip   ,traj2(k2)%cool_n_sip   ,traj2(k2+1)%cool_n_sip   ,dum)
       trajf(k1)%cool_n_op    = AVER(traj1(k1)%cool_n_op    ,traj2(k2)%cool_n_op    ,traj2(k2+1)%cool_n_op    ,dum)
       trajf(k1)%cool_n_np    = AVER(traj1(k1)%cool_n_np    ,traj2(k2)%cool_n_np    ,traj2(k2+1)%cool_n_np    ,dum)
       trajf(k1)%cool_n_sp    = AVER(traj1(k1)%cool_n_sp    ,traj2(k2)%cool_n_sp    ,traj2(k2+1)%cool_n_sp    ,dum)
       trajf(k1)%cool_n_h     = AVER(traj1(k1)%cool_n_h     ,traj2(k2)%cool_n_h     ,traj2(k2+1)%cool_n_h     ,dum)
       trajf(k1)%cool_n_c     = AVER(traj1(k1)%cool_n_c     ,traj2(k2)%cool_n_c     ,traj2(k2+1)%cool_n_c     ,dum)
       trajf(k1)%cool_n_si    = AVER(traj1(k1)%cool_n_si    ,traj2(k2)%cool_n_si    ,traj2(k2+1)%cool_n_si    ,dum)
       trajf(k1)%cool_n_o     = AVER(traj1(k1)%cool_n_o     ,traj2(k2)%cool_n_o     ,traj2(k2+1)%cool_n_o     ,dum)
       trajf(k1)%cool_n_n     = AVER(traj1(k1)%cool_n_n     ,traj2(k2)%cool_n_n     ,traj2(k2+1)%cool_n_n     ,dum)
       trajf(k1)%cool_n_s     = AVER(traj1(k1)%cool_n_s     ,traj2(k2)%cool_n_s     ,traj2(k2+1)%cool_n_s     ,dum)
       trajf(k1)%cool_n_fep   = AVER(traj1(k1)%cool_n_fep   ,traj2(k2)%cool_n_fep   ,traj2(k2+1)%cool_n_fep   ,dum)
  ...
  SUBROUTINE LINEAR_REGRESSION(N, X, Y, a, b, rsq)
    ...
    REAL(KIND=DP), DIMENSION(1:N)              :: Yf
    REAL(KIND=DP), DIMENSION(1:N)              :: e
    REAL(KIND=DP)                              :: xmoy
    REAL(KIND=DP)                              :: ymoy
    REAL(KIND=DP)                              :: sx
    REAL(KIND=DP)                              :: sxx
    ...
    ! xmoy = 1.0_dp / DBLE(N)   * SUM( X(1:N) )
    ! ymoy = 1.0_dp / DBLE(N)   * SUM( Y(1:N) )
    ! sx   = SQRT( 1.0_dp / DBLE(N-1) * SUM( (X(1:N)-xmoy)**2.0_dp ) )
    ! sy   = SQRT( 1.0_dp / DBLE(N-1) * SUM( (Y(1:N)-ymoy)**2.0_dp ) )
    ! sxy  = 1.0_dp / DBLE(N-1) * SUM( (X(1:N)-xmoy) * (Y(1:N)-ymoy) )
    ! r    = sxy / ( sx * sy )
    ...
    !    e (i) = Y(i) - Yf(i)
    ! ENDDO
    ! s = SQRT( 1.0_dp / DBLE(N-2) * SUM( e(1:N)**2.0_dp ) )
    ! SST = DBLE(N-1) * sy * sy
    ! SSR = DBLE(N-1) * sxy * sxy / ( sx * sx )
