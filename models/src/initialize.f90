...
CONTAINS
  ...
  SUBROUTINE INITIALIZE
    ...
    USE MODULE_DUST_TREATMENT, ONLY : READ_DRAINE_FILES, COMPUTE_QCOEF
    ...
    ! DO i = 1, Nspec
    !    WRITE(*,'(A8,1X,12(I2))') speci(i)%name, speci(i)%formula(1:Nelements)
    ! ENDDO
    ! WRITE(*,*)
    ! DO i = 1, Nelements
    ...
    IF ( (shock_type_init(1:1) == "S") .OR. (shock_type_init(1:1) == "P") ) THEN
       d_ero = 0.0_DP
    ELSE
       CALL COMPUTE_DERO
    ENDIF
    ...
    ! DO i = 1, nreact
    !    WRITE(*,*) speci(react(i)%r(1))%name,  speci(react(i)%r(2))%name, &
    !               speci(react(i)%p(1))%name,  speci(react(i)%p(2))%name, &
    !               speci(react(i)%p(3))%name,  speci(react(i)%p(4))%name, &
    !               react(i)%DE
    ! ENDDO
    ! STOP
