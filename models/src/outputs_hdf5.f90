...
CONTAINS
  ...
  SUBROUTINE WRITE_METADATA(filename, metatab, nhdf5, ifil, ndatafile, typehdf5)
    ...
    USE MODULE_VAR_VODE, ONLY : duration_max, length_max, Eps_V
    ...
    CHARACTER(len=lenstrint)                       :: strindex        ! integer index in string format
    ...
    INTEGER                                        :: i, j, k, l, j1, j2 ! dummy indices
    ...
    DO l = 1, nvals
       ...
       DO i = ifil, ifil + ndatafile - 1
          ...
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'gradv_i               ', .false.) == 1 ) ) THEN
             ...
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_tot            ', .false.) == 1 ) ) THEN  
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_tot
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_chem           ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_chem
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_diff_in        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_diff_in
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_diff_an        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_diff_an
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_diff_en        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_diff_en
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_diff_gn        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_diff_gn
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_inel_en        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_inel_en
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_ther_in        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_ther_in
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_ther_gn        ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_ther_gn
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_ph             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_ph
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_cr             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_cr
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_h2int          ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_h2int
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_visc           ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_visc
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'heat_n_compr          ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%heat_n_compr
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_tot            ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_tot
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_h2             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_h2
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_13co           ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_13co
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_oh             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_oh
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_nh3            ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_nh3
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_co             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_co
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_h2o            ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_h2o
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_cp             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_cp
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_sip            ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_sip
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_op             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_op
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_np             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_np
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_sp             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_sp
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_c              ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_c
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_si             ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_si
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_o              ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_o
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_n              ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_n
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_s              ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_s
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
          ELSE IF ( ( INDEX(metatab(i)%IDname, 'cool_n_fep            ', .false.) == 1 ) ) THEN
             WRITE(strf,'(1p,E16.8e3)') traj_main(k)%cool_n_fep
             WRITE(iwrtmp,'(A)',ADVANCE='no') TRIM(key_temp)//TRIM(strf)
