...
CONTAINS
  ...
  SUBROUTINE READ_PHOTODES
    ...
    CHARACTER(LEN=lenfilename)                            :: phodest_file ! Name of photodestruction file
    REAL(KIND=DP)                                         :: dum
    INTEGER                                               :: i, j, k
    INTEGER                                               :: nb_pt
  ...
  SUBROUTINE ENERGY_DEFECT_REACTION(i)
    ...
    DO j=1,3          ! count the occurence of the reactants *** modif Tabone 3Body 12/2017
       occ_reactants(react(i)%R(j)) = occ_reactants(react(i)%R(j))+1
    END DO
    DO j=1,4          ! count the occurence of the products
       occ_products(react(i)%P(j)) = occ_products(react(i)%P(j))+1
    END DO
    enthalpy_unknown = .FALSE.
    DO j=1,3          ! check for enthalpy of reactants *** modif Tabone 3Body 12/2017
    ...
    ! BUG CORRECTION - remove following line BG2017
  ...
  SUBROUTINE REACTION_TYPE
    ...
    !---------------------------------------------------
    ! (0) gaseous 3-body reactions  (type='3BODY') ******** Modif Tabone 12/2017 3Body
    !---------------------------------------------------
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 10 ) THEN
          ...
          react(N3body+ b_3body-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 40 ) THEN
          ...
          react(Nphele + b_phele-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 41 ) THEN
          ...
          react(Ngrrec + b_grrec-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 1 ) THEN
          ...
          CALL ENERGY_DEFECT_REACTION(Nh2_fo+ b_h2_fo-1) ! explicitely calculated
       ENDIF
    END DO
   ...
   !---------------------------------------------------
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 51 ) THEN
          ...
          react(Nphdes+ b_phdes-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 53 ) THEN
          ...
          react(Nsecde+ b_secde-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 54 ) THEN
          ...
          react(Nthdes+ b_thdes-1)%DE   = Zero
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 2 ) THEN
          ...
          ! Set DE to 0 in order to treat cooling by 
          ! collisional dissociation more properly
          ! in SOURCE (evolution.f90)
    ...
    DO i=1, Nreact
       IF ( react_aux(i)%itype == 0 .OR. &
          ...
          Nother = Nother + 1
          !---------------------------------------------
          ! Treatment of all standard chemical reactions
          !---------------------------------------------
          react(Nother+ b_other-1) = react_aux(i)
          IF (react_aux(i)%itype == 0 .OR. react_aux(i)%itype == 3 ) THEN
          ...
          !---------------------------------------------
          ! Treatment of reactions with excited H2
          !---------------------------------------------
...
!!!PIERRE!!!             STOP
  ...
  SUBROUTINE ADD_REVERSE_REACTIONS
    ...
    DO i=b_other,e_other
       ...
       DO WHILE ((j<=e_other) .AND. (.NOT. eliminate))
          ...
          reaction_aux%R(3)= 0              ! Tabone 12/2017 not ion/neutral 3body reactions!!!
  ...
  SUBROUTINE SORT_REACTIONS
    ...
    REAL(KIND=dp), DIMENSION(Nreact)  :: rate
    INTEGER,       DIMENSION(Nreact)  :: indx
    ...
    IF (first) THEN
       ...
       DO i = 1, Nreact
          ...
          speci(IR1)%nbreac = speci(IR1)%nbreac + 1
          speci(IR2)%nbreac = speci(IR2)%nbreac + 1
          speci(IR3)%nbreac = speci(IR3)%nbreac + 1   !modif Tabone 12/2017 3Body
          speci(IP1)%nbreac = speci(IP1)%nbreac + 1
          speci(IP2)%nbreac = speci(IP2)%nbreac + 1
          speci(IP3)%nbreac = speci(IP3)%nbreac + 1
          speci(IP4)%nbreac = speci(IP4)%nbreac + 1
       ENDDO
       DO j = 1, nspec
          ALLOCATE(speci(j)%sort_reac(speci(j)%nbreac,2))
          speci(j)%sort_reac(1:speci(j)%nbreac,1:2) = 0
       ENDDO
       first = .false.
    ENDIF
    ...
    DO j = 1, nspec
       ...
       DO i = 1, Nreact
          ...
          ENDIF
       ENDDO
       CALL SORT_TABLE(nb,rate(1:nb),indx(1:nb))
       ! -----------------------------------------
       ...
       DO i = 1, nb
          ...
          ! ENDIF
          ! --------------------------------------
          speci(j)%sort_reac(i,1) = indx(i)
          ! WRITE(*,*) speci(j)%sort_reac(i,1)
          IF(rate(i) < 0) THEN
             speci(j)%sort_reac(i,2) = -1
          ELSE
             speci(j)%sort_reac(i,2) = +1
          ENDIF
       ENDDO
