...
MODULE MODULE_GRAINS
  ...
  REAL(KIND=DP)            :: compr_gr                 ! grain compression factor
...
CONTAINS
  ...
  SUBROUTINE FUNC_DADS(x, f, df)
     ...
     f  = ( 3.0_DP * f3_mrn * x &
          + 3.0_DP * f2_mrn * x**2.0_DP &
          + f1_mrn * x**3.0_DP ) / f4_mrn &
        ! - rho_grc * d_site**3.0_DP / (Mgrc0 * compr_gr) *  ab_ads ! parametrization with d_site
        - rho_grc / rho_grm * Mgrm / (Mgrc0 * compr_gr) ! parametrization with rho_grm
     ...
     !      + f1_mrn * x**3.0_DP ) / f4_mrn &
     !    - rho_grc * d_site**3.0_DP / (Mgrc0 * compr_gr) *  ab_ads
     !    ! - rho_grc / rho_grm * Mgrm / (Mgrc0 * compr_gr) ! parametrization with rho_grm
...
MODULE MODULE_PHYS_VAR
  ...
  !--- shock ---
  CHARACTER(len=2)   :: shock_type_init    ! initial shock type ('S', 'P', 'C', or 'J')
  CHARACTER(len=2)   :: shock_type_lock    ! initial shock type ('S', 'P', 'C', or 'J')
  CHARACTER(len=2)   :: shock_type         ! current shock type ('S', 'P', 'C', or 'J')
  ...
  SUBROUTINE READ_PARAMETERS
    ...
    IF ( (shock_type_init(1:1) == "S") .OR. &
       ...
       IF ( Nfluids /= 1 ) THEN
          ...
          PRINT *,'    Nfluid must = 1 for shock_type_init = S, P, or J'
          STOP
       ENDIF
       ! --------------------
       ! old
