MODULE MODULE_CHEMICAL_SPECIES
  ...
  TYPE TYPE_ELEMENT
     ...
     REAL(KIND=DP)               :: Dens_init ! density (cm-3)
     REAL(KIND=DP)               :: ab        ! abundance
     REAL(KIND=DP)               :: ab_init   ! initial abundance
     REAL(KIND=DP)               :: ab_ref    ! abundance from Anders & Grevesse 1989
     INTEGER,       DIMENSION(2) :: idsp_mn   ! indices of the two main gas phase carriers
     REAL(KIND=DP), DIMENSION(2) :: absp_mn   ! elemental abundance of the two main gas phase carriers
  ...
  TYPE TYPE_SPECY
     ...
     INTEGER                      :: nbreac         ! number of chemical reaction (formation or destruction)
     INTEGER, DIMENSION(:,:), ALLOCATABLE :: sort_reac ! indices of reactions (sorted by increasing absolute rate)
  END TYPE TYPE_SPECY
...
CONTAINS
  ...
  SUBROUTINE READ_SPECIES
    ...
    REAL(KIND = DP)                   :: density_limit = 1.0e-20_DP
  ...
  SUBROUTINE ELEMENTAL_ABUNDANCES
    ...
    DO i=1, Nelements
       elements(i)%ab = SUM(DBLE(speci(:Nspec)%formula(i)) * speci(:Nspec)%density)
    END DO
  ...
  SUBROUTINE INDEX_CONSERVATION
    ...
    !---------------------------------------------------------------------------
    IMPLICIT NONE
    INTEGER(KIND=LONG) :: i, j, l, ii
    INTEGER(KIND=LONG) :: isp1, isp2
    ...
    ! --------------------------------------------
    ! DO i = 1, Nelements
    !    WRITE(*, '(A8,3X)', advance = 'no') elements(i)%name
    !    DO ii = 1, Nspec
    !       IF(speci(ii)%icon == i) EXIT
    !    ENDDO
    !    WRITE(*,*) speci(ii)%name
    ! ENDDO
    ! STOP
  ...
  SUBROUTINE CONSISTENCY
    ...
    !    elements are adjusted in order to insure conservation of initial
    ...
    ! -------------------------------------------
    ! Find the species used to conserve all elem
    ! -------------------------------------------
    kk = 0
    DO k = 1, Nelements
       IF(skipele(k)) CYCLE
    ...
    DO k = 1, Nelements
       IF(skipele(k)) CYCLE
       kk = kk + 1
       c_bbmn(kk,1) = elements(k)%Dens_init
       DO i = 1, Nspec
          IF (speci(i)%icon /= 0) CYCLE
          c_bbmn(kk,1) = c_bbmn(kk,1) - speci(i)%formula(k) * speci(c_ivar(kk))%density
       ENDDO
    ENDDO
    ...
    DO k = 1, Nelements
       ...
       IF(c_bbmx(kk,1) <= 0.0_dp) THEN
          ...
          ! WRITE(*,*) speci(c_ivar(kk))%name, c_bbmx(kk,1)
          ! c_bbmx(kk,1) = 1e-40_dp
          CYCLE
       ENDIF
       speci(c_ivar(kk))%density = c_bbmx(kk,1)
    ENDDO
